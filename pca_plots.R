...
alpha.meta <- left_join(alpha.frame, meta_map, by = 'SampleID') #combine metadata
...
ggplot(beta2.frame, aes(x = PC2, y = PC1, color = State_Region))+
  ...
  xlab(paste("PC1: ", round(100*weighted_pco$data$ProportionExplained[1]), "%"))+
  ylab(paste("PC2: ", round(100*weighted_pco$data$ProportionExplained[2]), "%"))+
